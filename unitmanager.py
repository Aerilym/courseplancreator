...
def import_subjects(data):
    ...
    eftsl = data[9]
    studylevel = data[10]
    opentoexchange = data[11]
    rawsemester = deepcopy(semdata)
    ...
    for h in range(len(reqdata)):
        for bit in reqdata[h]:
            if 'PREREQUISITE:' in bit:
                ...
                RAWprereqs[h] = copy(prereqs[h]).capitalize()
                if 'PREREQUISITE' in prereqs[h]:
                    prereqs[h] = prereqs[h].replace('PREREQUISITE', 'AND')
                if 'PREREQUISITE' in RAWprereqs[h]:
                    RAWprereqs[h] = RAWprereqs[h].replace('PREREQUISITE', 'and')
            elif 'COREQUISITE:' in bit:
                coreqs[h] = bit.replace('COREQUISITE:', '')
                RAWcoreqs[h] = copy(coreqs[h]).capitalize()
            elif 'PROHIBITION:' in bit:
                prohibs[h] = bit.replace('PROHIBITION:', '')
                RAWprohibs[h] = copy(prohibs[h]).capitalize()
            else:
    ...
    for innerlst in reqlst:
        for j in range(len(innerlst)):
            ...
            if 'RECOMMENDED' in req:
                ...
                if recom[j] == '':
                    RAWrecoms[j] = req[reco_i:].capitalize()
                    recom[j] = req[reco_i:]
                else:
                    RAWrecoms[j] += req[reco_i:].capitalize()
                    recom[j] += req[reco_i:]
    ...
    for i in range(len(RAWprereqs)):
        if len(RAWprereqs[i])>6:
            for j in range(0,len(RAWprereqs[i])-6):
                if is_unit(RAWprereqs[i][j:j+7], exactly=True):
                    RAWprereqs[i] = RAWprereqs[i][:j] + RAWprereqs[i][j:j+7].upper() + RAWprereqs[i][j+7:]
    for i in range(len(RAWcoreqs)):
        if len(RAWcoreqs[i])>6:
            for j in range(0,len(RAWcoreqs[i])-6):
                if is_unit(RAWcoreqs[i][j:j+7], exactly=True):
                    RAWcoreqs[i] = RAWcoreqs[i][:j] + RAWcoreqs[i][j:j+7].upper() + RAWcoreqs[i][j+7:]
    for i in range(len(RAWprohibs)):
        if len(RAWprohibs[i])>6:
            for j in range(0,len(RAWprohibs[i])-6):
                if is_unit(RAWprohibs[i][j:j+7], exactly=True):
                    RAWprohibs[i] = RAWprohibs[i][:j] + RAWprohibs[i][j:j+7].upper() + RAWprohibs[i][j+7:]
        

    return units, prereqs, coreqs, prohibs, recom, sems, namedata, overview, creditpoints, faculty, school, rawsemester, RAWprereqs, RAWcoreqs, RAWprohibs, RAWrecoms, scaband, eftsl, studylevel, opentoexchange
    


def prereq_error_handle(subjects,prereqs,addedunits):
    unmetreqs = []
    unmetsubjects = []
    ...
    for subject in subjects:
        if subject not in addedunits:
            unmetsubjects.append(subject)
    return unmetreqs, unmetsubjects


def prereq_check(prereqs,unitsdone_sem,points):
    for i in range(len(prereqs)):
        for j in range(len(prereqs[i])):
            for k in range(len(prereqs[i][j])):
                req = prereqs[i][j][k]
...
def coreqs_met(i,coreqs,unitsdone_sem,currentssemlst):
    if coreqs[i] == [['']]:
        return True
    for j in range(len(coreqs[i])):
        ...
        for k in range(len(coreqs[i][j])):
            coreq = coreqs[i][j][k]
            if coreq in unitsdone_sem:
                return True
            elif coreq in currentssemlst:
                return True
    return False

def prereqs_met(i,prereqs,unitsdone_sem, points):
    prereqscop = deepcopy(prereqs)
    if prereqscop[i] == [['']]:
        return True
    for j in range(len(prereqscop[i])):
        for k in range(len(prereqscop[i][j])):
            req = prereqscop[i][j][k]
            if req == '':
                prereqscop[i][j][k] = 'Met'
            elif req in unitsdone_sem:
                prereqscop[i][j][k] = 'Met'
            elif req[0].isnumeric():
                #expects in form '24toFIT' or '6L1FIT' or '36total'
                ...
                if int(pointreq) <= pointscurrent:
                    prereqscop[i][j][k] = 'Met'
    for j in range(len(prereqscop[i])):
        if prereqscop[i][j] == ['Met' for _ in range(len(prereqscop[i][j]))]:
            prereqscop[i][j] = 'Met'
        elif 'Met' in prereqscop[i][j]:
            prereqscop[i][j] = 'Met'
    if prereqscop[i] == ['Met' for _ in range(len(prereqscop[i]))]:
        return True
    return False
...
def allvalidtiers(currentsemlst, subjects, coreqs, maxinputlength = 10):
    def validgroup(group, subjects, coreqs):
        for unit in group:
            unit_i = subjects.index(unit)
            if coreqs[unit_i] == [['']]:
                return True
            for j in range(len(coreqs[unit_i])):
                if coreqs[unit_i][j] == []:
                        return True
                for k in range(len(coreqs[unit_i][j])):
                    coreq = coreqs[unit_i][j][k]
                    if coreq in group:
                        return True
            return False
    currentsemlst.sort(key=lambda x: x[3])
    if len(currentsemlst)>maxinputlength:
        currentsemlst = currentsemlst[:maxinputlength+1]
    validgroups = []
    comb = list(itertools.combinations(currentsemlst,4))
    for group in comb:
        if validgroup(group, subjects, coreqs):
...
def prereq_order(subjects, prereqs, coreqs, prohibs, recom, sems, done = [], semstart = 1):
    ...
    empcount = 0
    currenttier = 1
    maxcount = length**3 if length>3 else 50
    while len(addedunits) < length and count < maxcount:
        count += 1

        possibleunits = []
        for i in range(len(subjects)):
            if subjects[i] not in unitsdone_sem and prereqs_met(i,prereqs,unitsdone_sem,points) and (sems[i] == '*' or sems[i] == semester):
                possibleunits.append(i)

        currentsemlst = []
        concfails = 0
        while possibleunits != [] and concfails < 4:
            change = False
            for unit_i in possibleunits:
                if coreqs_met(unit_i,coreqs,unitsdone_sem,currentsemlst):
                    currentsemlst.append(subjects[unit_i])
            for i in range(len(possibleunits)-1,-1,-1):
                if subjects[possibleunits[i]] in currentsemlst:
                    possibleunits.pop(i)
                    change = True
            if not change:
                concfails += 1
                ...
                concfails = 0
        if len(currentsemlst) > 4:
            validtiers = allvalidtiers(currentsemlst, subjects, coreqs, 6)
            othertiers.append([currenttier,validtiers])
            tier = validtiers[0]
        else:
            tier = currentsemlst
            othertiers.append([currenttier,[tier]])
        print(tier, file=sys.stderr)
        if not tier:
            empcount += 1
            if empcount > 10:
                unmetreqs, unmetsubjects = prereq_error_handle(subjects,prereqs,addedunits)
                other.append('Error: Prereqs not met')
                other.append([unmetreqs,unmetsubjects])
                return order, semstart, unitsdone_sem, other, othertiers
        if tier:
            order.append(tier)
            currenttier += 1
            for sub in tier:
                unitsdone_sem.append(sub)
                addedunits.append(sub)
                pointhandle(sub,points)
        tier = []
        if semester == 1:
            semester = 2
...
#units, prereqs, coreqs, prohibs, recom, sems, unitnames = unitfinder(['MTH2010', 'MTH2032', 'PHS2062', 'FIT1049', 'FIT2094', 'FIT2081', 'FIT2001', 'FIT3175', 'ASP3051', 'PHS3201', 'FIT2002', 'ASP3012', 'PHS3000', 'FIT3047', 'FIT3077', 'PHS3101', 'ASP3231', 'FIT3048', 'FIT2099', 'FIT3146', 'ASP3162'])
#semesters = []
